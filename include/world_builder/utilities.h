...
namespace WorldBuilder
{
  ...
  namespace Utilities
  {
     ...
     * smallest distance of the point to the polygon. The sign is negative for
     * points outside of the polygon and positive for points inside the polygon.
     */
    double signed_distance_to_polygon(const std::vector<Point<2>> &point_list_,
     ...
     * cartesian (x,y,z) in meters and in spherical radius in meters and
     * longitude and latitude in radians. \param point_list This is a vector of
     * 2d Points in natural coordinates at the surface which define the line
     * along the surface at which the curved planes start. Natural coordinates
     * are in cartesian (x,y,z) in meters and in spherical radius in meters and
     * longitude and latitude in radians. \param plane_segment_lengths This is a
     * vector of vectors of doubles. It contains the length of every segment at
     * point in the point_list (in the same order as the point_list. \param
     * plane_segment_angles This is a vector of vectors of 2d points. It
     * contains the begin and end angle of every segment at point in the
     * point_list (in the same order as the point_list. \param start_radius This
     * value contains the radius or height from bottom of the box at which the
     * plane starts. This means that the start_radius effectively becomes the
     * surface for this slab.
     * \param coordinate_system This is a reference to the coordinate system of
     * the World Builder. This is used to convert cartesian to natural
     * coordinates and back. \param only_positive This value deterines whether
     * only the the part below the plane should count as distance or both sides
     * of the plane. It is called only_positive because the area below the
     * plane, the distance is positve, and above the plane the distance is
     * negative. \param interpolation_type This value determines what
     * interpolation type should be used when determining the location with
     * respect to the curved plane. \param spline_x the spline representing the
     * x coordinate. \param spline_y the spline representing the y coordinate.
     * \param global_x_list This is a list of one dimensional coorindates, with
     * zero or the amount of coordinates entries, used for interpolation. An
     * empty list is interpretated as a list filled with {0,1,2,...,number of
     * coordinates}. Filling this list with other values changes the returned
     * section fraction. It allows for, for example, adding extra coordinates
     * automatically, and still reference the user provided coordinates by the
     * original number. Note that no whole numbers may be skiped. So for a list
