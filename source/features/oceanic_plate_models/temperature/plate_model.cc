    ...
    namespace OceanicPlateModels
    {
      namespace Temperature
      {
                                         ...
                                         const std::string & /*unused*/)
        {
          ...
          prm.declare_entry("",
                            Types::Object({"ridge coordinates",
                                           "spreading velocity", "max depth"
                                          }),
                            "Temperature model object");

          prm.declare_entry("min depth", Types::Double(0),
                            "The depth in meters from which the temperature of "
                            "this feature is present.");

          prm.declare_entry("max depth",
                            Types::Double(std::numeric_limits<double>::max()),
                            "The depth in meters to which the temperature of "
                            "this feature is present.");
          ...
          prm.declare_entry(
            "spreading velocity", Types::Double(-1),
            "The spreading velocity of the plate in meter per year. "
            "This is the velocity with which one side moves away from the "
            "ridge.");

          prm.declare_entry(
            "ridge coordinates", Types::Array(Types::Point<2>(), 2),
        ...
        void PlateModel::parse_entries(Parameters &prm)
        {
          ...
          const double dtr =
            prm.coordinate_system->natural_coordinate_system() == spherical
            ? const_pi / 180.0
            : 1.0;
          ...
          if (depth <= max_depth && depth >= min_depth)
            {
              ...
              for (unsigned int i_ridge = 0;
                   i_ridge < ridge_coordinates.size() - 1; i_ridge++)
                {
                  ...
                  const Point<2> check_point(
                    natural_coordinate.get_surface_coordinates(),
                    natural_coordinate.get_coordinate_system());
                  ...
                  // This part is needed when we want to consider segments instead
                  // of lines If you want to have infinite lines, use only the else
                  // statement.
                  ...
                  compare_point[0] =
                    ...
                    ? Pb[0]
                    : natural_coordinate.get_depth_coordinate();
                  compare_point[1] = coordinate_system == cartesian ? Pb[1] : Pb[0];
                  compare_point[2] = coordinate_system == cartesian
                                     ? natural_coordinate.get_depth_coordinate()
                                     : Pb[1];

                  distance_ridge = std::min(
                                     distance_ridge,
                                     this->world->parameters.coordinate_system
                                     ->distance_between_points_at_same_depth(
                                       Point<3>(natural_coordinate.get_coordinates(),
                                                natural_coordinate.get_coordinate_system()),
                                       compare_point));
                }
              ...
              double temperature =
                top_temperature + (bottom_temperature_local - top_temperature) *
                (depth / max_depth);
              ...
              for (int i = 1; i < sommation_number + 1; ++i)
                {
                  temperature =
                    temperature +
                    (bottom_temperature_local - top_temperature) *
                    ((2 / (double(i) * const_pi)) *
                     std::sin((double(i) * const_pi * depth) / max_depth) *
                     std::exp(
                       (((spreading_velocity * max_depth) /
                         (2 * thermal_diffusivity)) -
                        std::sqrt(
                          ((spreading_velocity * spreading_velocity *
                            max_depth * max_depth) /
                           (4 * thermal_diffusivity *
                            thermal_diffusivity)) +
                          double(i) * double(i) * const_pi * const_pi)) *
                       ((spreading_velocity * age) / max_depth)));
                }

              WBAssert(!std::isnan(temperature),
                       "Temparture inside plate model is not a number: "
                       << temperature
                       << ". Relevant variables: bottom_temperature_local = "
                       << bottom_temperature_local << ", top_temperature = "
                       << top_temperature << ", max_depth = " << max_depth
                       << ", spreading_velocity = " << spreading_velocity
                       << ", thermal_diffusivity = " << thermal_diffusivity
                       << ", age = " << age << ".");
              WBAssert(std::isfinite(temperature),
                       "Temparture inside plate model is not a finite: "
                       << temperature
                       << ". Relevant variables: bottom_temperature_local = "
                       << bottom_temperature_local
        ...
        WB_REGISTER_FEATURE_OCEANIC_PLATE_TEMPERATURE_MODEL(PlateModel,
                                                            plate model)
      } // namespace Temperature
    }   // namespace OceanicPlateModels
  }     // namespace Features
