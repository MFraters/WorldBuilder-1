    ...
    namespace SubductingPlateModels
    {
      namespace Temperature
      {
        PlateModel::PlateModel(WorldBuilder::World *world_)
          : min_depth(NaN::DSNAN), max_depth(NaN::DSNAN), density(NaN::DSNAN),
            plate_velocity(NaN::DSNAN), thermal_conductivity(NaN::DSNAN),
            thermal_expansion_coefficient(NaN::DSNAN),
            specific_heat(NaN::DSNAN),
            potential_mantle_temperature(NaN::DSNAN),
            surface_temperature(NaN::DSNAN), adiabatic_heating(true),
        ...
        PlateModel::~PlateModel() = default;

        void PlateModel::declare_entries(Parameters &prm,
                                         const std::string & /*unused*/)
        {
          ...
          prm.declare_entry(
            "density", Types::Double(3300),
            "The reference density of the subducting plate in $kg/m^3$");

          prm.declare_entry("plate velocity", Types::Double(NaN::DQNAN),
                            "The velocity in meters per year with which the "
                            "plate subducts in meters per year.");

          prm.declare_entry("thermal conductivity", Types::Double(2.0),
                            "The thermal conductivity of the subducting plate "
                            "material in $W m^{-1} K^{-1}$.");

          prm.declare_entry(
            "thermal expansion coefficient", Types::Double(-1),
            "The thermal expansivity of the subducting plate material in "
            "$K^{-1}$. If smaller than zero, the global value is used.");

          prm.declare_entry("specific heat", Types::Double(-1),
                            "The specific heat of the subducting plate "
                            "material in $J kg^{-1} K^{-1}$. If smaller than "
                            "zero, the global value is used.");

          prm.declare_entry("adiabatic heating", Types::Bool(true),
                            "Wheter adiabatic heating should be used for the "
                            "slab. Setting the parameter to false leads to "
                            "equation 26 from McKenzie (1970),"
                            "which is the result obtained from McKenzie 1969.");

          prm.declare_entry(
            "potential mantle temperature", Types::Double(-1),
            "The potential temperature of the mantle at the surface in "
            "Kelvin. If smaller than zero, the global value is used.");
        }

        void PlateModel::parse_entries(Parameters &prm)
        {
          ...
          // spreading_velocity = prm.get<double>("spreading
          // velocity")/31557600; ridge_coordinates = prm.get_vector<Point<2>
          // >("ridge coordinates");
          ...
          if (thermal_expansion_coefficient < 0)
            thermal_expansion_coefficient =
              this->world->thermal_expansion_coefficient;
          ...
          potential_mantle_temperature =
            this->world->potential_mantle_temperature >= 0
        ...
        double PlateModel::get_temperature(
          ...
          const std::map<std::string, double> &distance_from_planes) const
        {
          const double thickness_local =
            std::min(distance_from_planes.at("thicknessLocal"), max_depth);
          const double distance_from_plane =
            distance_from_planes.at("distanceFromPlane");
          const double distance_along_plane =
            distance_from_planes.at("distanceAlongPlane");

          if (distance_from_plane <= max_depth &&
              distance_from_plane >= min_depth)
            {
              /*
              ...
              double x_scaled =
                (std::fabs(distance_along_plane) <
                 2.0 * std::numeric_limits<double>::epsilon()
                 ? 2.0 * std::numeric_limits<double>::epsilon()
                 : distance_along_plane / thickness_local);

              // the paper uses `(x_scaled * sin(average_angle) - z_scaled *
              // cos(average_angle))` to compute the depth (execpt that you do not
              // use average angles since they only have on angle). On recomputing
              // their result it seems to me (Menno) that it should have been
              // `(1-z_scaled)` instead of `z_scaled`. To avoid this whole problem
              // we just use the depth directly since we have that. todo: get the
              // local thickniss out of H, that prevents an other division. If we
              // want to specifiy the bottom temperature, because we have defined
              // a linear temperture increase in the mantle and/or oceanic plate,
              // we have to switch off adiabatic heating for now. Todo: there may
              // be a better way to deal with this.
              ;
              double temp = adiabatic_heating
                            ? std::exp(((thermal_expansion_coefficient *
                                         gravity_norm * depth) /
                                        specific_heat))
                            : 1;

              WBAssert(!std::isnan(temp),
                       "Internal error: temp is not a number: "
                       << temp << ". In exponent: "
                       << std::exp(((thermal_expansion_coefficient *
                                     gravity_norm) /
                                    specific_heat) *
              ...
              for (int i = 1; i <= n_sum; i++)
                {
                  sum += (std::pow((-1.0), i) / (i * const_pi)) *
                         (exp((R -
                               std::pow(R * R + i * i * const_pi * const_pi, 0.5)) *
                              x_scaled)) *
                         (sin(i * const_pi * z_scaled));
                }
              // todo: investiage wheter this 273.15 should just be the surface
              // temperature.
              double temperature =
                temp * (potential_mantle_temperature +
                        2.0 * (potential_mantle_temperature - 273.15) * sum);

              WBAssert(!std::isnan(temperature),
                       "Internal error: temperature is not a number: "
                       << temperature << ".");
              WBAssert(std::isfinite(temperature),
                       "Internal error: temperature is not finite: "
                       << temperature << ".");
        ...
        WB_REGISTER_FEATURE_SUBDUCTING_PLATE_TEMPERATURE_MODEL(PlateModel,
                                                               plate model)
      } // namespace Temperature
    }   // namespace SubductingPlateModels
  }     // namespace Features
