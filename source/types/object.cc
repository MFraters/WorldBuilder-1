...
namespace WorldBuilder
{
  namespace Types
  {
    ...
    void Object::write_schema(Parameters &prm, const std::string & /*name*/,
      ...
      WBAssertThrow(
          !prm.path.empty() && prm.path.back() == "properties",
      ...
      prm.leave_subsection();
      {
        ...
        Pointer((path + "/additionalProperties").c_str())
            .Set(declarations, additional_properties);

        if (!required.empty()) {
          for (unsigned int i = 0; i < required.size(); ++i) {
            if (i == 0 &&
                Pointer((path + "/required").c_str()).Get(declarations) ==
                    nullptr) {
              // The required array doesn't exist yet, so we create it and fill
              // it.
              Pointer((path + "/required/0").c_str()).Create(declarations);
              Pointer((path + "/required/0").c_str())
                  .Set(declarations, required[i].c_str());
            } else {
              // The required array already exist yet, so we add an element to
              // the end.
              Pointer((path + "/required/-").c_str())
                  .Set(declarations, required[i].c_str());
            }
          }
        }
