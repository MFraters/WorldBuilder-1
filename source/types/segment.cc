...
namespace WorldBuilder
{
  ...
  namespace Types
  {
    ...
    Segment::Segment(Segment const &other)
        : value_length(other.default_length),
          ...
          value_angle(other.default_angle), default_angle(other.default_angle),
    ...
    void Segment::write_schema(Parameters &prm, const std::string &name,
      ...
      prm.enter_subsection(name);
      {
        ...
        Pointer((base + "/additionalProperties").c_str())
            .Set(declarations, false);
        Pointer((base + "/documentation").c_str())
            .Set(declarations, documentation.c_str());
        std::vector<std::string> restricted_values = {"length", "thickness",
                                                      "angle"};
        for (unsigned int i = 0; i < restricted_values.size(); ++i) {
          if (!restricted_values[i].empty()) {
            if (i == 0 &&
                Pointer((base + "/required").c_str()).Get(declarations) ==
                    nullptr) {
              // The enum array doesn't exist yet, so we create it and fill it.
              Pointer((base + "/required/0").c_str()).Create(declarations);
              Pointer((base + "/required/0").c_str())
                  .Set(declarations, restricted_values[i].c_str());
            } else {
              // The enum array already exist yet, so we add an element to the
              // end.
              Pointer((base + "/required/-").c_str())
                  .Set(declarations, restricted_values[i].c_str());
            }
          }
        }
        ...
        prm.enter_subsection("properties");
        {
          ...
          Pointer((base + "/thickness/type").c_str())
              .Set(declarations, "array");
          Pointer((base + "/thickness/minItems").c_str()).Set(declarations, 1);
          Pointer((base + "/thickness/maxItems").c_str()).Set(declarations, 2);
          Pointer((base + "/thickness/items/type").c_str())
              .Set(declarations, "number");

          Pointer((base + "/top truncation/type").c_str())
              .Set(declarations, "array");
          Pointer((base + "/top truncation/minItems").c_str())
              .Set(declarations, 1);
          Pointer((base + "/top truncation/maxItems").c_str())
              .Set(declarations, 2);
          Pointer((base + "/top truncation/items/type").c_str())
              .Set(declarations, "number");
          ...
          Pointer((base + "/angle/items/type").c_str())
              .Set(declarations, "number");
  ...
  namespace Objects
  {
    ...
    template <class A, class B, class C>
    Segment<A, B, C>::~Segment() = default;

    template <class A, class B, class C>
    void Segment<A, B, C>::write_schema(
        Parameters & /*prm*/, const std::string & /*name*/,
        const std::string & /*documentation*/) const {
     ...
     * before this function is called.
     */
    template class Segment<Features::FaultModels::Temperature::Interface,
