...
namespace WorldBuilder
{
  namespace Utilities
  {
    bool polygon_contains_point(const std::vector<Point<2>> &point_list,
                                const Point<2> &point)
    {
      ...
      const std::vector<Point<2>> &point_list, const Point<2> &point)
    {
       ...
       * http://geomalgorithms.com/a03-_inclusion.html, and therefore requires
          ...
          if (point_list[j][1] <= point[1])
            {
              ...
              if (point_list[i][1] >= point[1]) // an upward crossing
                {
                  const double is_left = (point_list[i][0] - point_list[j][0]) *
                                         (point[1] - point_list[j][1]) -
                                         (point[0] - point_list[j][0]) *
                  ...
                  if (is_left < 0)
                    {
                      ...
                      --wn; // have  a valid down intersect
                           ...
                           std::numeric_limits<double>::epsilon())
                    {
                      // This code is to make sure that the boundaries are included in
                      // the polygon. The point is exactly on the infinite line.
                      // determine if it is on the segment
                      const double dot_product =
                        (point - point_list[j]) * (point_list[i] - point_list[j]);
    ...
    double signed_distance_to_polygon(const std::vector<Point<2>> &point_list,
                                      const Point<2> &point)
    {
      // If the point lies outside polygon, we give it a negative sign,
      // inside a positive sign.
      const double sign =
        polygon_contains_point(point_list, point) ? 1.0 : -1.0;
      ...
      WBAssertThrow(n_poly_points >= 3,
                    "Not enough polygon points were specified.");

      // Initialize a vector of distances for each point of the polygon with a
      // very large distance
      std::vector<double> distances(n_poly_points, 1e23);

      // Create another polygon but with all points shifted 1 position to the
      // right
      std::vector<Point<2>> shifted_point_list(
                           n_poly_points, Point<2>(point.get_coordinate_system()));
      ...
      for (size_t i = 0; i < n_poly_points; ++i)
        {
          ...
          // point lies closer to not-shifted polygon point, but perpendicular
          // base line lies outside segment
          if (c1 <= 0.0)
            distances[i] = (Point<2>(point_list[i] - point)).norm();
          // point lies closer to shifted polygon point, but perpendicular base
          // line lies outside segment
          else if (c2 <= c1)
            distances[i] = (Point<2>(shifted_point_list[i] - point)).norm();
          // perpendicular base line lies on segment
          else
            {
              const Point<2> point_on_segment =
                point_list[i] + (c1 / c2) * vector_segment;
              distances[i] = (Point<2>(point - point_on_segment)).norm();
            }
        }

      // Return the minimum of the distances of the point to all polygon
      // segments
      return *std::min_element(distances.begin(), distances.end()) * sign;
    }

    NaturalCoordinate::NaturalCoordinate(
      const std::array<double, 3> &position,
      const CoordinateSystems::Interface &coordinate_system_)
    {
      coordinate_system = coordinate_system_.natural_coordinate_system();
      coordinates =
        coordinate_system_.cartesian_to_natural_coordinates(position);
    }

    // todo, should be possible to make this without the interface, since the
    // Point knows the coord system.
    NaturalCoordinate::NaturalCoordinate(
      const Point<3> &position,
      const CoordinateSystems::Interface &coordinate_system_)
    {
      coordinate_system = coordinate_system_.natural_coordinate_system();
      coordinates = coordinate_system_.cartesian_to_natural_coordinates(
                      position.get_array());
    }
    ...
    CoordinateSystem NaturalCoordinate::get_coordinate_system() const
    {
    ...
    spherical_to_cartesian_coordinates(const std::array<double, 3> &scoord)
    {
      ...
      ccoord[0] = scoord[0] * std::sin(0.5 * const_pi - scoord[2]) *
                  std::cos(scoord[1]); // X
      ccoord[1] = scoord[0] * std::sin(0.5 * const_pi - scoord[2]) *
                  std::sin(scoord[1]);                              // Y
    ...
    unsigned int string_to_unsigned_int(const std::string &string)
    {
      ...
      if (!(i >> d) || i.get(c))
        WBAssertThrow(false,
                      "Could not convert \"" + s + "\" to an unsigned int.");
    ...
    {
      WBAssert(a.get_coordinate_system() == b.get_coordinate_system(),
               "Trying to do a cross product of points of a different "
               "coordinate system.");
    ...
    std::map<std::string, double> distance_point_from_curved_planes(
      const Point<3> &check_point,     // cartesian point in spherical system
      const Point<2> &reference_point, // in (rad) spherical coordinates in
      // spherical system
      const std::vector<Point<2>>
      &point_list, // in  (rad) spherical coordinates in spherical system
    ...
    {
      // TODO: Assert that point_list, plane_segment_angles and
      // plane_segment_lenghts have the same size.
      /*WBAssert(point_list.size() == plane_segment_lengths.size(),
               "Internal error: The size of point_list (" << point_list.size()
               << ") and plane_segment_lengths (" <<
      plane_segment_lengths.size() << ") are different.");
      WBAssert(point_list.size() == plane_segment_angles.size(),
               "Internal error: The size of point_list (" << point_list.size()
               << ") and plane_segment_angles (" << plane_segment_angles.size()
      << ") are different."); WBAssert(point_list.size() ==
      plane_segment_angles.size(), "Internal error: The size of point_list (" <<
      point_list.size()
               << ") and global_x_list (" << global_x_list.size() << ") are
      different.");*/
      ...
      WBAssertThrow(global_x_list.size() == point_list.size(),
                    "The given global_x_list doesn't have "
      ...
      const CoordinateSystem natural_coordinate_system =
        coordinate_system->natural_coordinate_system();
      const bool bool_cartesian = natural_coordinate_system == cartesian;

      const Point<3> check_point_natural(
        coordinate_system->cartesian_to_natural_coordinates(
          check_point.get_array()),
        natural_coordinate_system);
      const Point<3> check_point_surface(
        bool_cartesian ? check_point_natural[0] : start_radius,
        check_point_natural[1],
        bool_cartesian ? start_radius : check_point_natural[2],
        natural_coordinate_system);
      const Point<2> check_point_surface_2d(
        bool_cartesian ? check_point_natural[0] : check_point_natural[1],
      ...
      WBAssertThrow(depth_method == DepthMethod::none ||
                    depth_method ==
                    DepthMethod::angle_at_starting_point_with_surface ||
                    depth_method ==
                    DepthMethod::angle_at_begin_segment_with_surface,
                    "Only the depth methods none, "
                    "angle_at_starting_point_with_surface and "
      ...
      if (interpolation_type != InterpolationType::ContinuousMonotoneSpline)
        {
          // loop over all the planes to find out which one is closest to the
          // point.
          for (size_t i_section = 0; i_section < point_list.size() - 1;
               ++i_section)
            {
              ...
              closest_point_on_line_2d_temp =
                P1 + ((P1PC * P1P2) / (P1P2 * P1P2)) * P1P2;
              ...
              double fraction_CPL_P1P2_strict_temp =
                (P1CPL * P1P2 <= 0 ? -1.0 : 1.0) *
                (1 - (P1P2.norm() - P1CPL.norm()) / P1P2.norm());

              double min_distance_check_point_surface_2d_line_temp =
                closest_point_on_line_2d_temp.cheap_relative_distance(
                  check_point_surface_2d); //(closest_point_on_line_2d_temp -
              //check_point_surface_2d).norm();//closest_point_on_line_2d_temp.distance(check_point_surface_2d);
              // If fraction_CPL_P1P2_strict_temp is between 0 and 1 it means that
              // the point can be projected perpendicual to the line segment. For
              // the non-contiuous case we only conder points which are
              // perpendicular to a line segment.
              // There can be mutliple lines segment to which a point is
              // perpundicual. Choose the point which is closed in 2D (x-y).
              if (fraction_CPL_P1P2_strict_temp >= 0. &&
                  fraction_CPL_P1P2_strict_temp <= 1. &&
                  fabs(min_distance_check_point_surface_2d_line_temp) <
                  fabs(min_distance_check_point_surface_2d_line))
                {
                  min_distance_check_point_surface_2d_line =
                    min_distance_check_point_surface_2d_line_temp;
          ...
          continue_computation =
            (fabs(fraction_CPL_P1P2_strict) < INFINITY &&
             fraction_CPL_P1P2_strict >= 0. && fraction_CPL_P1P2_strict <= 1.);

          fraction_CPL_P1P2 =
            global_x_list[i_section_min_distance] -
            static_cast<int>(global_x_list[i_section_min_distance]) +
            (global_x_list[i_section_min_distance + 1] -
             global_x_list[i_section_min_distance]) *
            fraction_CPL_P1P2_strict;
        }
      else
        {
          ...
          Point<2> splines(x_spline(min_estimate_solution),
                           y_spline(min_estimate_solution),
                           natural_coordinate_system);
          double minimum_distance_to_reference_point =
            splines.cheap_relative_distance(check_point_surface_2d);

          // Compute the clostest point on the spline as a double.
          for (size_t i_estimate = 0;
               i_estimate <=
               static_cast<size_t>(
                 parts * (global_x_list[point_list.size() - 1]) + 1);
               i_estimate++)
            {
              splines[0] = x_spline(min_estimate_solution_temp);
              splines[1] = y_spline(min_estimate_solution_temp);
              const double minimum_distance_to_reference_point_temp =
                splines.cheap_relative_distance(check_point_surface_2d);

              if (fabs(minimum_distance_to_reference_point_temp) <
                  fabs(minimum_distance_to_reference_point))
                {
                  minimum_distance_to_reference_point =
                    minimum_distance_to_reference_point_temp;
                  min_estimate_solution = min_estimate_solution_temp;
                }
              min_estimate_solution_temp = min_estimate_solution_temp + 1.0 / parts;
            }

          // search above and below the solution and replace if the distance is
          // smaller.
          ...
          for (size_t i_search_step = 0; i_search_step < 10; i_search_step++)
            {
              ...
              const double minimum_distance_to_reference_point_min =
                splines.cheap_relative_distance(check_point_surface_2d);
              ...
              if (minimum_distance_to_reference_point_plus <
                  minimum_distance_to_reference_point)
                {
                  min_estimate_solution = min_estimate_solution + search_step;
                  minimum_distance_to_reference_point =
                    minimum_distance_to_reference_point_plus;
                }
          ...
          continue_computation =
            (solution > 0 &&
             floor(solution) <= global_x_list[point_list.size() - 2] &&
             floor(solution) >= 0);

          closest_point_on_line_2d = Point<2>(
                                       x_spline(solution), y_spline(solution), natural_coordinate_system);
      ...
      if (continue_computation)
        {
          ...
          const Point<3> closest_point_on_line_surface(
            bool_cartesian ? closest_point_on_line_2d[0] : start_radius,
            bool_cartesian ? closest_point_on_line_2d[1]
            : closest_point_on_line_2d[0],
          ...
          WBAssert(!std::isnan(closest_point_on_line_bottom[0]) ||
                   ...
                   "Internal error: The y_axis variable contains not a number: "
                   << closest_point_on_line_bottom);

          // Now that we have both the check point and the
          // closest_point_on_line, we need to push them to cartesian.
          Point<3> closest_point_on_line_cartesian(
            coordinate_system->natural_to_cartesian_coordinates(
              closest_point_on_line_surface.get_array()),
            cartesian);
          Point<3> closest_point_on_line_bottom_cartesian(
            coordinate_system->natural_to_cartesian_coordinates(
              closest_point_on_line_bottom.get_array()),
            cartesian);
          Point<3> check_point_surface_cartesian(
            coordinate_system->natural_to_cartesian_coordinates(
              check_point_surface.get_array()),
            cartesian);

          WBAssert(!std::isnan(closest_point_on_line_bottom_cartesian[0]),
                   "Internal error: The y_axis variable is not a number: "
                   << closest_point_on_line_bottom_cartesian[0]);
          ...
          size_t original_current_section = static_cast<size_t>(
                                              std::floor(global_x_list[i_section_min_distance]));
          size_t original_next_section = original_current_section + 1;

          // These are the mostly likely cases for the x and y axis, so initialize
          // them to these values. They will be checked in the else statement or
          // replaced in the if statement.
          Point<3> y_axis = closest_point_on_line_cartesian -
                            closest_point_on_line_bottom_cartesian;
          Point<3> x_axis =
            closest_point_on_line_cartesian - check_point_surface_cartesian;

          // This are accouting for corner cases.
          // If the point to check is exactly on or below the line, we can not
          // compute the x-axis with this method. We could use an other method
          // where we use the two point before and after it, but we can also just
          // nudge it into a direction, which seems to work very well.
          if (std::fabs(
                (check_point_surface - closest_point_on_line_surface).norm()) <
              2e-14)
            {
              // If the point to check is on the line, we don't need to search any
              // further, because we know the distance is zero.
              if (std::fabs(
                    (check_point - closest_point_on_line_cartesian).norm()) >
                  2e-14)
                {
                  ...
                  const Point<2>
                  closest_point_on_line_plus_normal_to_plane_spherical =
                    closest_point_on_line_2d +
                    1e-8 *
                    (closest_point_on_line_2d.norm() > 1.0
                     ? closest_point_on_line_2d.norm()
                     : 1.0) *
                    unit_normal_to_plane_spherical;

                  WBAssert(
                    std::fabs(closest_point_on_line_plus_normal_to_plane_spherical
                              .norm()) > std::numeric_limits<double>::epsilon(),
                    "Internal error: The norm of variable "
                    "'closest_point_on_line_plus_normal_to_plane_spherical' "
                    "is  zero, while this may not happen.");

                  const Point<3>
                  closest_point_on_line_plus_normal_to_plane_surface_spherical(
                    bool_cartesian
                    ? closest_point_on_line_plus_normal_to_plane_spherical
                    [0]
                    : start_radius,
                    ...
                    [1],
                    natural_coordinate_system);
                  const Point<3> closest_point_on_line_plus_normal_to_plane_cartesian(
                    coordinate_system->natural_to_cartesian_coordinates(
                      closest_point_on_line_plus_normal_to_plane_surface_spherical
                      .get_array()),
                    cartesian);
                  Point<3> normal_to_plane =
                    closest_point_on_line_plus_normal_to_plane_cartesian -
                    closest_point_on_line_cartesian;
                  normal_to_plane = normal_to_plane / normal_to_plane.norm();

                  // The y-axis is from the bottom/center to the
                  // closest_point_on_line, the x-axis is 90 degrees rotated from
                  // that, so we rotate around the line P1P2. Todo: Assert that the
                  // norm of the axis are not equal to zero.
                  y_axis = closest_point_on_line_cartesian -
                           closest_point_on_line_bottom_cartesian;

                  WBAssert(std::abs(y_axis.norm()) >
                           std::numeric_limits<double>::epsilon(),
                           "World Builder error: Cannot detemine the up direction in "
                           "the model. This is most likely due to the provided start "
                           "radius being zero."
                           << " Techical details: The y_axis.norm() is zero. "
                           "Y_axis is "
                           << y_axis[0] << ":" << y_axis[1] << ":" << y_axis[2]
                           << ". closest_point_on_line_cartesian = "
                           << closest_point_on_line_cartesian[0] << ":"
                  ...
                  x_axis = Point<3>(
                             ux * ux * vx + ux * uy * vy - uz * vy + uy * uz * vz + uy * vz,
                             uy * ux * vx + uz * vx + uy * uy * vy + uy * uz * vz - ux * vz,
                             uz * ux * vx - uy * vx + uz * uy * vy + ux * vy + uz * uz * vz,
                             cartesian);

                  // see on what side the line P1P2 reference point is. This is based
                  // on the determinant
                  const double reference_on_side_of_line =
                    (point_list[i_section_min_distance + 1][0] -
                     point_list[i_section_min_distance][0]) *
                    (reference_point[1] -
                     point_list[i_section_min_distance][1]) -
                    (point_list[i_section_min_distance + 1][1] -
                     point_list[i_section_min_distance][1]) *
                    (reference_point[0] -
                     point_list[i_section_min_distance][0]) <
                    0
                  ...
                  WBAssert(!std::isnan(x_axis[0]),
                           "Internal error: The x_axis variable is not a number: "
                           << x_axis[0]);
              ...
              else
                {
                  total_average_angle =
                    ...
                    fraction_CPL_P1P2 *
                    (plane_segment_angles[original_next_section][0][0] -
                     plane_segment_angles[original_current_section][0][0]);
                  ...
                  return_values["section"] =
                    static_cast<double>(i_section_min_distance);
          ...
          else
            {
              ...
              WBAssert(
                std::abs(y_axis.norm()) > std::numeric_limits<double>::epsilon(),
                "World Builder error: Cannot detemine the up direction in the "
                "model. This is most likely due to the provided start radius "
                "being zero."
                << " Techical details: The y_axis.norm() is zero. Y_axis is "
                << y_axis << ". closest_point_on_line_cartesian = "
                << closest_point_on_line_cartesian
                << ", closest_point_on_line_bottom_cartesian = "
                << closest_point_on_line_bottom_cartesian);
              ...
              // check whether the check point and the reference point are on the
              // same side, if not, change the side.
              const bool reference_on_side_of_line_bool =
                (point_list[i_section_min_distance + 1][0] -
                 point_list[i_section_min_distance][0]) *
                (reference_point[1] -
                 point_list[i_section_min_distance][1]) -
                ...
                (reference_point[0] -
                 point_list[i_section_min_distance][0]) <
                0;
              const bool checkpoint_on_side_of_line_bool =
                (point_list[i_section_min_distance + 1][0] -
                 point_list[i_section_min_distance][0]) *
                (check_point_surface_2d[1] -
                 point_list[i_section_min_distance][1]) -
                (point_list[i_section_min_distance + 1][1] -
                 point_list[i_section_min_distance][1]) *
                (check_point_surface_2d[0] -
                 point_list[i_section_min_distance][0]) <
                0;
              const double reference_on_side_of_line =
                reference_on_side_of_line_bool == checkpoint_on_side_of_line_bool
                ? -1
                : 1;
          ...
          WBAssert(!std::isnan(x_axis[0]),
                   "Internal error: The x_axis[0] variable is not a number: "
                   << x_axis[0] << ". Relevant values:  check_point = "
                   << check_point << ".");
          WBAssert(!std::isnan(x_axis[1]),
                   "Internal error: The x_axis[1] variable is not a number: "
                   << x_axis[1]);
          WBAssert(!std::isnan(x_axis[2]),
                   "Internal error: The x_axis[2] variable is not a number: "
                   << x_axis[2]);

          // now that we have the x and y axes computed, convert the 3d check
          // point into a 2d one.
          Point<2> check_point_2d(
            x_axis * (check_point - closest_point_on_line_bottom_cartesian),
            y_axis * (check_point - closest_point_on_line_bottom_cartesian),
            cartesian);

          Point<2> begin_segment(
            x_axis * (closest_point_on_line_cartesian -
                      closest_point_on_line_bottom_cartesian),
            y_axis * (closest_point_on_line_cartesian -
                      closest_point_on_line_bottom_cartesian),
            cartesian);
          ...
          WBAssert(!std::isnan(check_point_2d[0]),
                   "Internal error: The check_point_2d variable is not a number: "
                   << check_point_2d[0]);
          ...
          WBAssert(!std::isnan(begin_segment[0]),
                   "Internal error: The begin_segment variable is not a number: "
                   << begin_segment[0]);
          ...
          for (size_t i_segment = 0;
               i_segment < plane_segment_lengths[original_current_section].size();
               i_segment++)
            {
              ...
              if (i_segment != 0 &&
                  ...
                  DepthMethod::angle_at_begin_segment_with_surface)
                {
                  const double add_angle_inner =
                    (begin_segment * end_segment) /
                    (begin_segment.norm() * end_segment.norm());

                  WBAssert(
                    !std::isnan(add_angle_inner),
                    "Internal error: The add_angle_inner variable is not a number: "
                    << add_angle_inner << ". Variables: begin_segment = "
                    << begin_segment << ", end_segment = " << end_segment
                    << ", begin_segment * end_segment / (begin_segment.norm() "
                    "* end_segment.norm()) = "
                    << std::setprecision(32)
                    << begin_segment * end_segment /
                    (begin_segment.norm() * end_segment.norm())
                  ...
                  // there could be round of error problems here is the inner part is
                  // close to one
                  WBAssert(add_angle_inner >= 0 && add_angle_inner <= 1,
                           "Internal error: The variable add_angle_inner is smaller "
                           "than zero or larger then one,"
                           "which causes the std::acos to return nan. If it is only "
                           "a little bit larger then one, "
                           "this is probably caused by that begin and end segment "
                           "are the same and round off error. "
                           "The value of add_angle_inner = "
                           << add_angle_inner);
                  ...
                  WBAssert(
                    !std::isnan(add_angle),
                    "Internal error: The add_angle variable is not a number: "
                    << add_angle << ". Variables: begin_segment = "
                    << begin_segment << ", end_segment = " << end_segment
                    << ", begin_segment * end_segment / (begin_segment.norm() "
                    "* end_segment.norm()) = "
                    << std::setprecision(32)
                    << begin_segment * end_segment /
                    (begin_segment.norm() * end_segment.norm())
                    << ", std::acos(begin_segment * end_segment / "
                    "(begin_segment.norm() * end_segment.norm())) = "
                    << std::acos(begin_segment * end_segment /
                                 (begin_segment.norm() * end_segment.norm())));
                }
              ...
              WBAssert(
                !std::isnan(begin_segment[0]),
                "Internal error: The begin_segment variable is not a number: "
                << begin_segment[0]);
              WBAssert(
                !std::isnan(begin_segment[1]),
                "Internal error: The begin_segment variable is not a number: "
                << begin_segment[1]);
              ...
              WBAssert(plane_segment_angles.size() > original_next_section,
                       "Error: original_next_section = "
                       << original_next_section
                       << ", and plane_segment_angles.size() = "
                       << plane_segment_angles.size());

              WBAssert(plane_segment_angles[original_next_section].size() >
                       current_segment,
                       "Error: current_segment = "
                       << current_segment << ", and current_segment.size() = "
                       << plane_segment_angles[original_next_section].size());

              const double interpolated_angle_top =
                plane_segment_angles[original_current_section][current_segment]
                [0] +
                fraction_CPL_P1P2 *
                (plane_segment_angles[original_next_section][current_segment]
                 [0] -
                 plane_segment_angles[original_current_section]
                 [current_segment][0]) +
                add_angle;

              const double interpolated_angle_bottom =
                plane_segment_angles[original_current_section][current_segment]
                [1] +
                fraction_CPL_P1P2 *
                (plane_segment_angles[original_next_section][current_segment]
              ...
              double interpolated_segment_length =
                plane_segment_lengths[original_current_section][current_segment] +
                fraction_CPL_P1P2 *
                (plane_segment_lengths[original_next_section]
                 [current_segment] -
                 plane_segment_lengths[original_current_section]
                 [current_segment]);
              WBAssert(!std::isnan(interpolated_angle_top),
                       "Internal error: The interpolated_angle_top variable is not "
                       "a number: "
                       << interpolated_angle_top);
              ...
              const double difference_in_angle_along_segment =
                interpolated_angle_top - interpolated_angle_bottom;
              ...
              if (std::fabs(difference_in_angle_along_segment) < 1e-8)
                {
                  ...
                  if (std::fabs(interpolated_segment_length) >
                      std::numeric_limits<double>::epsilon())
                    {
                      end_segment[0] +=
                        interpolated_segment_length *
                      ...
                      Point<2> normal_2d_plane(-begin_end_segment[0],
                                               begin_end_segment[1], cartesian);
                      WBAssert(std::fabs(normal_2d_plane.norm()) >
                               std::numeric_limits<double>::epsilon(),
                               "Internal Error: normal_2d_plane.norm() is zero, which "
                               "should not happen. "
                               << "Extra info: begin_end_segment[0] = "
                               << begin_end_segment[0]
                               << ", begin_end_segment[1] = "
                               << begin_end_segment[1] << ", end_segment: ["
                               << end_segment[0] << "," << end_segment[1] << "]"
                      ...
                      if (c1 < 0 || c2 < c1)
                      ...
                      else
                        {
                          ...
                          const double side_of_line =
                            (begin_segment[0] - end_segment[0]) *
                            (check_point_2d[1] - begin_segment[1]) -
                            (begin_segment[1] - end_segment[1]) *
                            (check_point_2d[0] - begin_segment[0]) <
                            0
                            ? -1.0
                            : 1.0;
              ...
              else
                {
                  ...
                  const double radius_angle_circle =
                    std::fabs(interpolated_segment_length /
                              difference_in_angle_along_segment);

                  WBAssert(!std::isnan(radius_angle_circle),
                           "Internal error: The radius_angle_circle variable is not "
                           "a number: "
                           << radius_angle_circle
                           << ". interpolated_segment_length = "
                           << interpolated_segment_length
                  ...
                  WBAssert(!std::isnan(cos_angle_top),
                           "Internal error: The radius_angle_circle variable is not "
                           "a number: "
                           << cos_angle_top << ". interpolated_angle_top = "
                           << interpolated_angle_top);
                  ...
                  if (std::fabs(interpolated_angle_top - 0.5 * const_pi) < 1e-8)
                    {
                      ...
                      center_circle[0] = difference_in_angle_along_segment > 0
                                         ? begin_segment[0] + radius_angle_circle
                                         : begin_segment[0] - radius_angle_circle;
                      center_circle[1] = begin_segment[1];
                    }
                  else if (std::fabs(interpolated_angle_top - 1.5 * const_pi) <
                           1e-8)
                    {
                  ...
                  else
                    {
                      ...
                      WBAssert(
                        !std::isnan(tan_angle_top),
                        "Internal error: The tan_angle_top variable is not a number: "
                        << tan_angle_top);
                      const double center_circle_y =
                        difference_in_angle_along_segment < 0
                        ? begin_segment[1] - radius_angle_circle * cos_angle_top
                        : begin_segment[1] + radius_angle_circle * cos_angle_top;

                      WBAssert(!std::isnan(center_circle_y),
                               "Internal error: The center_circle_y variable is not a "
                               "number: "
                               << center_circle_y
                      ...
                      WBAssert(!std::isnan(CCYBS),
                               "Internal error: The CCYBS variable is not a number: "
                               << CCYBS);
                  ...
                  WBAssert(
                    !std::isnan(center_circle[0]) || !std::isnan(center_circle[1]),
                    "Internal error: The center variable contains not a number: "
                    << center_circle[0] << ":" << center_circle[0]);
                  WBAssert(
                    std::fabs((begin_segment - center_circle).norm() -
                              std::fabs(radius_angle_circle)) <
                    1e-8 * std::fabs((begin_segment - center_circle).norm() +
                                     std::fabs(radius_angle_circle)),
                    "Internal error: The center of the circle is not a radius away "
                  ...
                  const double sin_angle_diff =
                    sin(difference_in_angle_along_segment);
                  const double cos_angle_diff =
                    cos(difference_in_angle_along_segment);
                  end_segment[0] = cos_angle_diff * BSPC[0] -
                                   sin_angle_diff * BSPC[1] + center_circle[0];
                  end_segment[1] = sin_angle_diff * BSPC[0] +
                                   cos_angle_diff * BSPC[1] + center_circle[1];

                  WBAssert(std::fabs((end_segment - center_circle).norm() -
                                     std::fabs(radius_angle_circle)) <
                           1e-8 * std::fabs((end_segment - center_circle).norm() +
                                            std::fabs(radius_angle_circle)),
                           "Internal error: The center of the circle is not a radius "
                           ...
                           << "The computed radius is "
                           << std::fabs((end_segment - center_circle).norm())
                           << ", and it should be " << radius_angle_circle
                           << ".");
                  ...
                  const double dot_product =
                    CPCR * Point<2>(0, radius_angle_circle, cartesian);
                  ...
                  double check_point_angle =
                    std::fabs(CPCR_norm) < std::numeric_limits<double>::epsilon()
                    ? 2.0 * const_pi
                    : (check_point_2d[0] <= center_circle[0]
                       ? std::acos(dot_product /
                                   (CPCR_norm * radius_angle_circle))
                       : 2.0 * const_pi -
                       std::acos(dot_product /
                                 (CPCR_norm * radius_angle_circle)));
                  check_point_angle = difference_in_angle_along_segment >= 0
                                      ? const_pi - check_point_angle
                                      : 2.0 * const_pi - check_point_angle;

                  // In the case that it is exactly 2 * pi, bring it back to zero
                  check_point_angle =
                    (std::fabs(check_point_angle - 2 * const_pi) < 1e-14
                     ? 0
                     : check_point_angle);

                  if ((difference_in_angle_along_segment > 0 &&
                       (check_point_angle <= interpolated_angle_top ||
                        std::fabs(check_point_angle - interpolated_angle_top) <
                        1e-12) &&
                       (check_point_angle >= interpolated_angle_bottom ||
                      ...
                      (difference_in_angle_along_segment < 0 &&
                       ...
                       (check_point_angle <= interpolated_angle_bottom ||
                        ...
                        1e-12)))
                    {
                      new_distance = (radius_angle_circle - CPCR_norm) *
                                     (difference_in_angle_along_segment < 0 ? 1 : -1);
                      new_along_plane_distance =
                        (radius_angle_circle * check_point_angle -
                         radius_angle_circle * interpolated_angle_top) *
                        (difference_in_angle_along_segment < 0 ? 1 : -1);
                    }
                }
              ...
              if (new_along_plane_distance >= -1e-10 &&
                  ...
                  std::fabs(new_distance) < std::fabs(distance))
                {
                  ...
                  segment = i_segment;
                  segment_fraction =
                    new_along_plane_distance / interpolated_segment_length;
                  total_average_angle =
              ...
              average_angle = (average_angle * total_length +
                               0.5 *
                               (interpolated_angle_top +
                                interpolated_angle_bottom - 2 * add_angle) *
                               interpolated_segment_length);
              average_angle =
                (std::fabs(average_angle) < std::numeric_limits<double>::epsilon()
                 ? 0
                 : average_angle /
      ...
      return_values["segment"] = static_cast<double>(segment);
      return_values["averageAngle"] = total_average_angle;
      return return_values;
    }
    ...
    {
      WBAssert(!x.empty(),
               "Internal error: The x in the set points function is zero.");
          ...
          for (size_t i = 0; i < n - 2; i++)
            {
              ...
              if (m0 * m1 <= 0)
              ...
              else
                {
                  ...
                  m_c[i + 1] =
                    3 * common / ((common + dx0) / m0 + (common + dx1) / m1);
                }
            }
          m_c[n - 1] = ms[n - 2];
    ...
    double interpolation::operator()(const double x) const
    {
      ...
      // Todo: The following two lines would work if m_x can be assumed to be
      // [0,1,2,3,...] Which would allow to optimize m_x away completely. I can
      // only do that once I get rid of the non-contiuous interpolation schemes,
      // because the contiuous one doesn't need any extra items in m_x.
      // const size_t idx = std::min((size_t)std::max( (int)x,
      // (int)0),mx_size_min); const double h = x-m_x[idx];
      // find the closest point m_x[idx] < x, idx=0 even if x<m_x[0]
      std::vector<double>::const_iterator it;
      it = std::lower_bound(m_x.begin(), m_x.end(), x);
      size_t idx = static_cast<size_t>(
                     std::max(static_cast<int>(it - m_x.begin()) - 1, 0));

      double h = x - m_x[idx];
      return (((x >= m_x[0] && x <= m_x[mx_size_min] ? m_a[idx] * h : 0) +
               m_b[idx]) *
              h +
              m_c[idx]) *
             h +
      ...
      const std::array<std::array<double, 3>, 3> &rotation_matrix)
    {
      ...
      // const double s2 = std::sqrt(rotation_matrix[2][1] *
      // rotation_matrix[2][1] + rotation_matrix[2][0] * rotation_matrix[2][0]);
      std::ostringstream os;
      for (size_t i = 0; i < 3; i++)
        for (size_t j = 0; j < 3; j++)
          WBAssert(std::fabs(rotation_matrix[i][j]) <= 1.0,
                   "rotation_matrix[" + std::to_string(i) + "][" +
                   std::to_string(j) + "] is larger than one: " +
      ...
      const double phi1 = std::atan2(rotation_matrix[2][0] / -sin(theta),
                                     rotation_matrix[2][1] / -sin(theta));
      const double phi2 = std::atan2(rotation_matrix[0][2] / -sin(theta),
    ...
    euler_angles_to_rotation_matrix(double phi1_d, double theta_d,
                                    double phi2_d)
    {
      ...
      rot_matrix[0][0] =
        cos(phi2) * cos(phi1) - cos(theta) * sin(phi1) * sin(phi2);
      rot_matrix[0][1] =
        -cos(phi2) * sin(phi1) - cos(theta) * cos(phi1) * sin(phi2);
      rot_matrix[0][2] = -sin(phi2) * sin(theta);

      rot_matrix[1][0] =
        sin(phi2) * cos(phi1) + cos(theta) * sin(phi1) * cos(phi2);
      rot_matrix[1][1] =
        -sin(phi2) * sin(phi1) + cos(theta) * cos(phi1) * cos(phi2);
    ...
    convert_point_to_array<2>(const Point<2> &point_);
    template std::array<double, 3>
    convert_point_to_array<3>(const Point<3> &point_);
  } // namespace Utilities
} // namespace WorldBuilder
