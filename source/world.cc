...
namespace WorldBuilder
{
  ...
  World::World(std::string filename, bool has_output_dir,
               const std::string &output_dir, unsigned long random_number_seed)
      : parameters(*this), surface_coord_conversions(invalid), dim(NaN::ISNAN),
  ...
  void World::declare_entries(Parameters &prm) {
    ...
    {
      prm.declare_entry("", Types::Object({"version", "features"}),
                        "Root object");

      prm.declare_entry("version", Types::String(""),
                        "The major and minor version number for which the "
                        "input file was written.");

      prm.declare_entry("cross section", Types::Array(Types::Point<2>(), 2, 2),
                        "This is an array of two points along where the cross "
                        "section is taken");

      prm.declare_entry(
          "potential mantle temperature", Types::Double(1600),
      ...
      prm.declare_entry(
          "force surface temperature", Types::Bool(false),
      ...
      prm.declare_entry(
          "maximum distance between coordinates", Types::Double(0),
          "This enforces a maximum distance (in degree for spherical "
          "coordinates "
          "or meter in cartesian coordinates) between coordinates in the "
          "model. "
          "If the distance is larger, extra points are added by interpolation. "
          "Requires interpolation to be not 'none'.");

      prm.declare_entry(
          "interpolation", Types::String("none"),
          "What type of interpolation should be used to enforce the minimum "
          "points per "
      ...
      prm.declare_entry(
          "coordinate system",
          Types::PluginSystem("cartesian",
                              CoordinateSystems::Interface::declare_entries,
                              {"model"}, false),
          "A coordinate system. Cartesian or spherical.");

      prm.declare_entry(
          "features",
          Types::PluginSystem("", Features::Interface::declare_entries,
                              {"model", "coordinates"}),
          "A list of features.");
    }
  ...
  void World::parse_entries(Parameters &prm) {
    ...
    WBAssertThrow(
        (Version::MAJOR == "0" && prm.get<std::string>("version") ==
                                      Version::MAJOR + "." + Version::MINOR) ||
            (Version::MAJOR != "0" &&
             prm.get<std::string>("version") == Version::MAJOR),
        "The major and minor version combination (for major version 0) or the "
        "major "
        "version (for major versions after 0) for which is input file was "
        "written "
        "is not the same as the version of the World Builder you are running. "
        "This means "
        "That there may have been incompatible changes made between the "
        "versions. \n\n"
        "Verify those changes and wheter they affect your model. If this is "
        "not "
        "the case, adjust the version number in the input file. \n\nThe "
        "provided version "
        "number is \""
            << prm.get<std::string>("version")
            << "\", while the used world builder "
               "has  (major.minor) version \""
            << Version::MAJOR << "." << Version::MINOR
            << "\". "
               "If you created this file from scratch, fill set the version "
               "number to \""
            << Version::MAJOR << "." << Version::MINOR
            << "\" to continue. If you got the world builder "
               "file from somewhere, make sure that the output is what you "
               "expect it to be, because "
               "backwards incompatible changes may have been made to the "
               "code.");

    /**
     * Seconly load the coordinate system parameters.
     */
    prm.coordinate_system =
        prm.get_unique_pointer<CoordinateSystems::Interface>(
            "coordinate system");
    ...
    if (set_cross_section) {
      dim = 2;
      std::vector<Point<2>> cross_section_natural =
          prm.get_vector<Point<2>>("cross section");

      WBAssertThrow(
          cross_section_natural.size() == 2,
          "The cross section should contain two points, but it contains "
              << cross_section.size() << " points.");

      for (const auto &it : cross_section_natural)
        cross_section.push_back(
            it * (coordinate_system == spherical ? const_pi / 180.0 : 1.0));
    ...
    thermal_expansion_coefficient =
        prm.get<double>("thermal expansion coefficient");
    ...
    maximum_distance_between_coordinates =
        prm.get<double>("maximum distance between coordinates");
  ...
  double World::temperature(const std::array<double, 2> &point,
                            const double depth,
                            const double gravity_norm) const {
    // turn it into a 3d coordinate and call the 3d temperature function
    WBAssertThrow(dim == 2,
                  "This function can only be called when the cross section "
                  "variable in the world builder file has been set. Dim is "
                      << dim << ".");

    const CoordinateSystem coordinate_system =
        this->parameters.coordinate_system->natural_coordinate_system();
    ...
    if (coordinate_system == spherical) {
      ...
      coord_3d[1] =
          cross_section[0][0] + point_natural[0] * surface_coord_conversions[0];
      coord_3d[2] =
    ...
    std::array<double, 3> point_3d_cartesian =
        this->parameters.coordinate_system->natural_to_cartesian_coordinates(
            coord_3d.get_array());

    return temperature(point_3d_cartesian, depth, gravity_norm);
  }

  double World::temperature(const std::array<double, 3> &point_,
                            const double depth,
                            const double gravity_norm) const {
    // We receive the cartesian points from the user.
    Point<3> point(point_, cartesian);

    if (std::fabs(depth) < 2.0 * std::numeric_limits<double>::epsilon() &&
    ...
    double temperature =
        potential_mantle_temperature *
        std::exp(
            ((thermal_expansion_coefficient * gravity_norm) / specific_heat) *
            depth);
    ...
    for (auto &&it : parameters.features) {
      ...
      WBAssert(!std::isnan(temperature),
               "Temparture is not a number: "
                   << temperature << ", based on a feature with the name "
                   << it->get_name());
    ...
    WBAssert(!std::isnan(temperature),
             "Temparture is not a number: " << temperature);
    WBAssert(std::isfinite(temperature),
             "Temparture is not a finite: " << temperature);

    return temperature;
  }

  double World::composition(const std::array<double, 2> &point,
                            const double depth,
  ...
  double World::composition(const std::array<double, 3> &point_,
                            const double depth,
                            const unsigned int composition_number) const {
    // We receive the cartesian points from the user.
    Point<3> point(point_, cartesian);
    ...
    for (auto &&it : parameters.features) {
      composition =
          it->composition(point, depth, composition_number, composition);

      WBAssert(!std::isnan(composition),
               "Composition is not a number: "
                   << composition << ", based on a feature with the name "
                   << it->get_name());
    ...
    WBAssert(!std::isnan(composition),
             "Composition is not a number: " << composition);
    WBAssert(std::isfinite(composition),
  ...
  WorldBuilder::grains World::grains(const std::array<double, 2> &point,
                                     const double depth,
  ...
  WorldBuilder::grains World::grains(const std::array<double, 3> &point_,
                                     const double depth,
                                     const unsigned int composition_number,
                                     size_t number_of_grains) const {
    // We receive the cartesian points from the user.
    ...
    for (const auto &feature : parameters.features) {
      ...
      WBAssert(std::isfinite(composition), "Composition is not a finite: " <<
      composition
               << ", based on a feature with the name " << (*it)->get_name());*/
    }

    /*WBAssert(!std::isnan(composition), "Composition is not a number: " <<
    composition); WBAssert(std::isfinite(composition), "Composition is not a
    finite: " << composition);*/

    return grains;
  }

  std::mt19937 &World::get_random_number_engine() {
