              ...
              for (size_t point_i = 0; point_i < angle_contraints[curve_i].size(); ++point_i)
                {
                  ...
                  directions[curve_i].emplace_back(contour_curves[curve_i+1].closest_point_on_curve(points[0][point_i]).point);
                }
            }
        }
              ...
              if (std::isnan(angle_contraints[0][point_i]))
                {
                  ...
                  ClosestPointOnCurve closest_point_below = contour_curves[1].closest_point_on_curve(points[0][point_i]);
                  ClosestPointOnCurve closest_point_below_below = contour_curves[2].closest_point_on_curve(closest_point_below.point);
              ...
              for (size_t point_i = 0; point_i < angle_contraints[curve_i].size(); ++point_i)
                {
                  if (std::isnan(angle_contraints[curve_i][point_i]))
                    {
                      // The first angle at each point is determined by the angle of the closest point below and the closest point below that
                      ClosestPointOnCurve closest_point_above = contour_curves[curve_i-1].closest_point_on_curve(points[curve_i][point_i]);
                      ClosestPointOnCurve closest_point_below = contour_curves[curve_i+1].closest_point_on_curve(points[curve_i][point_i]);
          ...
          for (size_t point_i = 0; point_i < angle_contraints[last_index].size(); ++point_i)
            {
              if (std::isnan(angle_contraints[last_index][point_i]))
                {
                  // The first angle at each point is determined by the angle of the closest point below and the closest point below that
                  ClosestPointOnCurve closest_point_above = contour_curves[last_index-1].closest_point_on_curve(points[last_index][point_i]);
                  ClosestPointOnCurve closest_point_above_above = contour_curves[last_index-2].closest_point_on_curve(closest_point_above.point);
                  ...
                  if (angle <= lowest_angle)
                    {
                      ...
                      const double curve_length = BezierCurve(local_points,local_angle_contraints).lengths[0];
