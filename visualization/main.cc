...
void project_on_sphere(double radius, double &x_, double &y_, double &z_) {
  ...
  double y = y_;
  double z = z_;
  const WorldBuilder::Point<3> in_point(
      std::array<double, 3>{{x, y, z}},
      WorldBuilder::CoordinateSystem::cartesian);
  WorldBuilder::Point<3> output_point(
...
void lay_points(double x1, double y1, double z1, double x2, double y2,
                double z2, double x3, double y3, double z3, double x4,
                double y4, double z4, std::vector<double> &x,
  ...
  for (size_t j = 0; j < level + 1; ++j) {
    for (size_t i = 0; i < level + 1; ++i) {
      ...
      const double x0 = -pi4 + static_cast<double>(i) * 2.0 *
                                   static_cast<double>(pi4) /
                                   static_cast<double>(level);
      const double y0 = -pi4 + static_cast<double>(j) * 2.0 *
                                   static_cast<double>(pi4) /
                                   static_cast<double>(level);
...
bool find_command_line_option(char **begin, char **end,
...
int main(int argc, char **argv) {
  ...
  try {
    ...
    if (find_command_line_option(argv, argv + argc, "-h") ||
        find_command_line_option(argv, argv + argc, "--help")) {
      std::cout << "This program allows to use the world builder library "
                   "directly with a world builder file and a grid file. "
                   "The data file will be filled with intitial conditions from "
                   "the world as set by the world builder file."
                << std::endl
                << "Besides providing two files, where the first is the world "
                   "builder file and the second is the grid file, the "
                   "available options are: "
                << std::endl
                << "-h or --help to get this help screen," << std::endl
                << "-j the number of threads the visualizer is allowed to use."
                << std::endl;
      return 0;
    }

    std::vector<std::string> options_vector =
        get_command_line_options_vector(argc, argv);

    for (size_t i = 0; i < options_vector.size(); ++i) {
      if (options_vector[i] == "-j") {
        number_of_threads =
            Utilities::string_to_unsigned_int(options_vector[i + 1]);
        options_vector.erase(
            options_vector.begin() +
    ...
    if (options_vector.empty()) {
      std::cout << "Error: There where no files passed to the World Builder, "
                   "use --help for more "
                << std::endl
                << "information on how  to use the World Builder app."
                << std::endl;
      return 0;
    }

    if (options_vector.size() == 1) {
      std::cout << "Error:  The World Builder app requires at least two files, "
                   "a World Builder file "
                << std::endl
    ...
    if (options_vector.size() != 2) {
      std::cout << "Only two command line arguments may be given, which should "
                   "be the world builder file location and the grid file "
  ...
  if (MPI_RANK == 0) {
     ...
     */
    std::cout << "[2/6] Starting the world builder with " << number_of_threads
              << " threads...                         \r";
    std::cout.flush();
    ...
    try {
      ...
      std::cerr << "Could not start the World builder from file '" << wb_file
                << "', error: " << e.what() << "\n";
    ...
    WBAssertThrow(
        data_stream.good(),
        "Could not find the provided convig file at the specified location: " +
            data_file);
    ...
    while (std::getline(data_stream, temp)) {
      ...
      for (auto &line_i : line)
        line_i.erase(std::remove(line_i.begin(), line_i.end(), ','),
                     line_i.end());
    ...
    WBAssertThrow(dim == 2 || dim == 3,
                  "dim should be set in the grid file and can only be 2 or 3.");

    WBAssertThrow(
        !std::isnan(x_min),
        "x_min is not a number:"
            << x_min
            << ". This value has probably not been provided in the grid file.");
    WBAssertThrow(
        !std::isnan(x_max),
        "x_max is not a number:"
            << x_max
            << ". This value has probably not been provided in the grid file.");
    WBAssertThrow(
        dim == 2 || !std::isnan(y_min),
        "y_min is not a number:"
            << y_min
            << ". This value has probably not been provided in the grid file.");
    WBAssertThrow(
        dim == 2 || !std::isnan(y_max),
        "y_max is not a number:"
            << y_max
            << ". This value has probably not been provided in the grid file.");
    WBAssertThrow(
        !std::isnan(z_min),
        "z_min is not a number:"
            << z_min
            << ". This value has probably not been provided in the grid file.");
    WBAssertThrow(
        !std::isnan(z_max),
        "z_max is not a number:"
            << z_max
            << ". This value has probably not been provided in the grid file.");

    WBAssertThrow(n_cell_x != 0,
                  "n_cell_z may not be equal to zero: " << n_cell_x << ".");
    // int's cannot generally be nan's (see
    // https://stackoverflow.com/questions/3949457/can-an-integer-be-nan-in-c),
    // but visual studio is giving problems over this, so it is taken out for
    // now.
    // WBAssertThrow(!std::isnan(n_cell_x), "n_cell_z is not a number:" <<
    // n_cell_x << ".");

    WBAssertThrow(dim == 3 || n_cell_z != 0,
    ...
    // but visual studio is giving problems over this, so it is taken out for
    // now.
    // WBAssertThrow(!std::isnan(n_cell_z), "n_cell_z is not a number:" <<
    // n_cell_y << ".");

    WBAssertThrow(n_cell_z != 0,
    ...
    if (grid_type == "sphere")
      WBAssert(n_cell_x == n_cell_y,
               "For the sphere grid the amount of cells in the x (long) and y "
               "(lat) direction have to be the same.");

    if (grid_type == "spherical" || grid_type == "chunk" ||
        grid_type == "anullus") {
      x_min *= (const_pi / 180);
      x_max *= (const_pi / 180);
      y_min *= (const_pi / 180);
    ...
    if (grid_type == "cartesian") {
      ...
      if (!compress_size && dim == 3)
        n_p = n_cell *
              8; // it shouldn't matter for 2d in the output, so just do 3d.
      else
      ...
      WBAssertThrow(dim == 2 || !std::isnan(dy),
      ...
      if (dim == 2) {
        for (size_t j = 0; j <= n_cell_z; ++j) {
          for (size_t i = 0; i <= n_cell_x; ++i) {
            ...
            grid_depth[counter] =
                (surface - z_min) - static_cast<double>(j) * dz;
            counter++;
          }
        }
      } else {
          ...
          for (size_t i = 0; i <= n_cell_x; ++i) {
            for (size_t j = 0; j <= n_cell_y; ++j) {
              for (size_t k = 0; k <= n_cell_z; ++k) {
                ...
                grid_depth[counter] =
                    (surface - z_min) - static_cast<double>(k) * dz;
                counter++;
              }
            }
          }
          ...
          for (size_t i = 0; i < n_cell_x; ++i) {
            for (size_t j = 0; j < n_cell_y; ++j) {
              for (size_t k = 0; k < n_cell_z; ++k) {
                ...
                grid_depth[counter] =
                    (surface - z_min) - (static_cast<double>(k) + 1.0) * dz;
                counter++;
                // position 1 of this cell
                grid_x[counter] = x_min + (static_cast<double>(i) + 1.0) * dx;
                grid_y[counter] = y_min + static_cast<double>(j) * dy;
                ...
                grid_depth[counter] =
                    (surface - z_min) - (static_cast<double>(k) + 1.0) * dz;
                WBAssert(counter < n_p,
                         "Assert counter smaller then n_P: counter = "
                             << counter << ", n_p = " << n_p);
                counter++;
              }
            }
      ...
      if (dim == 2) {
        for (size_t j = 1; j <= n_cell_z; ++j) {
          for (size_t i = 1; i <= n_cell_x; ++i) {
            ...
            grid_connectivity[counter][1] =
                i + 1 + (j - 1) * (n_cell_x + 1) - 1;
          ...
          for (size_t i = 1; i <= n_cell_x; ++i) {
            for (size_t j = 1; j <= n_cell_y; ++j) {
              for (size_t k = 1; k <= n_cell_z; ++k) {
                grid_connectivity[counter][0] =
                    (n_cell_y + 1) * (n_cell_z + 1) * (i - 1) +
                    (n_cell_z + 1) * (j - 1) + k - 1;
                grid_connectivity[counter][1] =
                    (n_cell_y + 1) * (n_cell_z + 1) * (i) +
                    (n_cell_z + 1) * (j - 1) + k - 1;
                grid_connectivity[counter][2] =
                    (n_cell_y + 1) * (n_cell_z + 1) * (i) +
                    (n_cell_z + 1) * (j) + k - 1;
                grid_connectivity[counter][3] =
                    (n_cell_y + 1) * (n_cell_z + 1) * (i - 1) +
                    (n_cell_z + 1) * (j) + k - 1;
                grid_connectivity[counter][4] =
                    (n_cell_y + 1) * (n_cell_z + 1) * (i - 1) +
                    (n_cell_z + 1) * (j - 1) + k;
                grid_connectivity[counter][5] =
                    (n_cell_y + 1) * (n_cell_z + 1) * (i) +
                    (n_cell_z + 1) * (j - 1) + k;
                grid_connectivity[counter][6] =
                    (n_cell_y + 1) * (n_cell_z + 1) * (i) +
                    (n_cell_z + 1) * (j) + k;
                grid_connectivity[counter][7] =
                    (n_cell_y + 1) * (n_cell_z + 1) * (i - 1) +
                    (n_cell_z + 1) * (j) + k;
                counter++;
              }
      ...
      size_t n_cell_t =
          static_cast<size_t>((2.0 * const_pi * outer_radius) / dr);

      // compute the ammount of cells
      n_cell = n_cell_t * n_cell_z;
      n_p =
          n_cell_t *
          (n_cell_z + 1); // one less then cartesian because two cells overlap.
      ...
      for (size_t j = 1; j <= n_cell_z + 1; ++j) {
        for (size_t i = 1; i <= n_cell_t; ++i) {
          ...
          grid_depth[counter] =
              outer_radius - std::sqrt(grid_x[counter] * grid_x[counter] +
                                       grid_z[counter] * grid_z[counter]);
          counter++;
        }
      }
      ...
      WBAssertThrow(inner_radius < outer_radius,
                    "The inner radius must be less than the outer radius.");

      WBAssertThrow(x_min - x_max <= 2.0 * const_pi,
                    "The difference between the minimum and maximum longitude "
      ...
      WBAssertThrow(
          y_min <= 0.5 * const_pi,
          "The maximum latitude must be smaller then or equal to 90 degree.");
      ...
      if (dim == 2) {
        for (size_t i = 1; i <= n_cell_x + 1; ++i)
          for (size_t j = 1; j <= n_cell_z + 1; ++j) {
            ...
            grid_z[counter] =
                inner_radius + (static_cast<double>(j) - 1.0) * dr;
          ...
          for (size_t i = 1; i <= n_cell_x + 1; ++i)
            for (size_t j = 1; j <= n_cell_y + 1; ++j)
              for (size_t k = 1; k <= n_cell_z + 1; ++k) {
                grid_x[counter] =
                    x_min + (static_cast<double>(i) - 1.0) * dlong;
                grid_y[counter] = y_min + (static_cast<double>(j) - 1.0) * dlat;
                grid_z[counter] =
                    inner_radius + (static_cast<double>(k) - 1.0) * dr;
      ...
      if (dim == 2) {
        for (size_t i = 1; i <= n_cell_x; ++i) {
          for (size_t j = 1; j <= n_cell_z; ++j) {
            ...
            counter = counter + 1;
            std::cout << "[4/6] Building the grid: stage 3 of 3 ["
      ...
      std::vector<std::vector<double>> block_grid_x(
          n_block, std::vector<double>(block_n_p));
      std::vector<std::vector<double>> block_grid_y(
          n_block, std::vector<double>(block_n_p));
      std::vector<std::vector<double>> block_grid_z(
          n_block, std::vector<double>(block_n_p));
      std::vector<std::vector<std::vector<size_t>>> block_grid_connectivity(
          n_block, std::vector<std::vector<size_t>>(
                       block_n_cell, std::vector<size_t>(block_n_v)));
      std::vector<std::vector<bool>> block_grid_hull(
          n_block, std::vector<bool>(block_n_p));
      ...
      for (size_t i_block = 0; i_block < n_block; ++i_block) {
        ...
        for (size_t j = 0; j <= block_n_cell_y; ++j) {
          for (size_t i = 0; i <= block_n_cell_y; ++i) {
            block_grid_x[i_block][counter] =
                static_cast<double>(i) * Lx /
                static_cast<double>(block_n_cell_x);
            block_grid_y[i_block][counter] =
                static_cast<double>(j) * Ly /
                static_cast<double>(block_n_cell_y);
        ...
        counter = 0;
        // using i=1 and j=1 here because i an j are not used in lookup and
        // storage so the code can remain very similar to ghost and the
        // cartesian code.
        for (size_t j = 1; j <= block_n_cell_y; ++j) {
          for (size_t i = 1; i <= block_n_cell_x; ++i) {
            block_grid_connectivity[i_block][counter][0] =
                i + (j - 1) * (block_n_cell_x + 1) - 1;
            block_grid_connectivity[i_block][counter][1] =
                i + 1 + (j - 1) * (block_n_cell_x + 1) - 1;
            block_grid_connectivity[i_block][counter][2] =
                i + 1 + j * (block_n_cell_x + 1) - 1;
            block_grid_connectivity[i_block][counter][3] =
                i + j * (block_n_cell_x + 1) - 1;
            counter++;
          }
        }
      }
      ...
      lay_points(xM, yM, zM, xG, yG, zG, xA, yA, zA, xE, yE, zE,
                 block_grid_x[0], block_grid_y[0], block_grid_z[0],
                 block_grid_hull[0], n_cell_x);
      lay_points(xF, yF, zF, xM, yM, zM, xE, yE, zE, xB, yB, zB,
                 block_grid_x[1], block_grid_y[1], block_grid_z[1],
                 block_grid_hull[1], n_cell_x);
      lay_points(xC, yC, zC, xG, yG, zG, xM, yM, zM, xF, yF, zF,
                 block_grid_x[2], block_grid_y[2], block_grid_z[2],
                 block_grid_hull[2], n_cell_x);
      lay_points(xG, yG, zG, xN, yN, zN, xJ, yJ, zJ, xA, yA, zA,
                 block_grid_x[3], block_grid_y[3], block_grid_z[3],
                 block_grid_hull[3], n_cell_x);
      lay_points(xC, yC, zC, xH, yH, zH, xN, yN, zN, xG, yG, zG,
                 block_grid_x[4], block_grid_y[4], block_grid_z[4],
                 block_grid_hull[4], n_cell_x);
      lay_points(xH, yH, zH, xD, yD, zD, xJ, yJ, zJ, xN, yN, zN,
                 block_grid_x[5], block_grid_y[5], block_grid_z[5],
                 block_grid_hull[5], n_cell_x);
      lay_points(xA, yA, zA, xJ, yJ, zJ, xP, yP, zP, xE, yE, zE,
                 block_grid_x[6], block_grid_y[6], block_grid_z[6],
                 block_grid_hull[6], n_cell_x);
      lay_points(xJ, yJ, zJ, xD, yD, zD, xK, yK, zK, xP, yP, zP,
                 block_grid_x[7], block_grid_y[7], block_grid_z[7],
                 block_grid_hull[7], n_cell_x);
      lay_points(xP, yP, zP, xK, yK, zK, xB, yB, zB, xE, yE, zE,
                 block_grid_x[8], block_grid_y[8], block_grid_z[8],
                 block_grid_hull[8], n_cell_x);
      lay_points(xQ, yQ, zQ, xK, yK, zK, xD, yD, zD, xH, yH, zH,
                 block_grid_x[9], block_grid_y[9], block_grid_z[9],
                 block_grid_hull[9], n_cell_x);
      lay_points(xQ, yQ, zQ, xH, yH, zH, xC, yC, zC, xF, yF, zF,
                 block_grid_x[10], block_grid_y[10], block_grid_z[10],
                 block_grid_hull[10], n_cell_x);
      lay_points(xQ, yQ, zQ, xF, yF, zF, xB, yB, zB, xK, yK, zK,
                 block_grid_x[11], block_grid_y[11], block_grid_z[11],
                 block_grid_hull[11], n_cell_x);

      // make sure all points end up on a sphere
      for (size_t i_block = 0; i_block < n_block; ++i_block) {
        for (size_t i_point = 0; i_point < block_n_p; ++i_point) {
          project_on_sphere(radius, block_grid_x[i_block][i_point],
                            block_grid_y[i_block][i_point],
                            block_grid_z[i_block][i_point]);
        }
      }
      ...
      // TODO: This becomes problematic with too large values of outer radius.
      // Find a better way, maybe through an epsilon.
      ...
      std::vector<std::vector<size_t>> shell_grid_connectivity(
          shell_n_cell, std::vector<size_t>(shell_n_v));
      ...
      for (size_t i = 0; i < n_block; ++i) {
        counter = 0;
        for (size_t j = i * block_n_cell; j < i * block_n_cell + block_n_cell;
             ++j) {
          for (size_t k = 0; k < shell_n_v; ++k) {
            shell_grid_connectivity[j][k] =
                block_grid_connectivity[i][counter][k] + i * block_n_p;
          }
          counter++;
        }
      }
      ...
      for (size_t i = 0; i < shell_n_cell; ++i) {
        for (size_t j = 0; j < shell_n_v; ++j) {
          shell_grid_connectivity[i][j] =
              point_to[shell_grid_connectivity[i][j]];
        }
      }
      ...
      for (size_t i = 0; i < shell_n_cell; ++i) {
        for (size_t j = 0; j < shell_n_v; ++j) {
          shell_grid_connectivity[i][j] =
              compact[shell_grid_connectivity[i][j]];
        }
      }
      ...
      for (size_t i = 0; i < n_cell_z + 1; ++i) {
        ...
        for (size_t j = 0; j < shell_n_p; ++j) {
          WBAssert(j < temp_shell_grid_x.size(),
                   "ERROR: j = " << j << ", temp_shell_grid_x.size() = "
                                 << temp_shell_grid_x.size());
          WBAssert(j < temp_shell_grid_y.size(),
                   "ERROR: j = " << j << ", temp_shell_grid_y.size() = "
                                 << temp_shell_grid_y.size());
          WBAssert(j < temp_shell_grid_z.size(),
                   "ERROR: j = " << j << ", temp_shell_grid_z.size() = "
                                 << temp_shell_grid_z.size());
          project_on_sphere(radius, temp_shell_grid_x[j], temp_shell_grid_y[j],
                            temp_shell_grid_z[j]);
        }
        ...
        for (size_t j = i_beg; j < i_end; ++j) {
          WBAssert(j < grid_x.size(),
                   "ERROR: j = " << j << ", grid_x.size() = " << grid_x.size());
          WBAssert(j < grid_y.size(),
                   "ERROR: j = " << j << ", grid_y.size() = " << grid_y.size());
          WBAssert(j < grid_z.size(),
                   "ERROR: j = " << j << ", grid_z.size() = " << grid_z.size());
          grid_x[j] = temp_shell_grid_x[counter];
          grid_y[j] = temp_shell_grid_y[counter];
          grid_z[j] = temp_shell_grid_z[counter];
          grid_depth[j] = outer_radius - std::sqrt(grid_x[j] * grid_x[j] +
                                                   grid_y[j] * grid_y[j] +
                                                   grid_z[j] * grid_z[j]);
      ...
      for (size_t i = 0; i < n_cell_z; ++i) {
        ...
        for (size_t j = i_beg; j < i_end; ++j) {
          for (size_t k = 0; k < shell_n_v; ++k) {
            grid_connectivity[j][k] =
                shell_grid_connectivity[counter][k] + i * shell_n_p;
          }
          counter++;
        }
        ...
        for (size_t j = i_beg; j < i_end; ++j) {
          for (size_t k = shell_n_v; k < 2 * shell_n_v; ++k) {
            WBAssert(k - shell_n_v < shell_grid_connectivity[counter].size(),
                     "k - shell_n_v is larger then "
                     "shell_grid_connectivity[counter]: k= "
                         << k << ", shell_grid_connectivity[counter].size() = "
                         << shell_grid_connectivity[counter].size());
            grid_connectivity[j][k] =
                shell_grid_connectivity[counter][k - shell_n_v] +
                (i + 1) * shell_n_p;
          }
          counter++;
        }
      ...
      WBAssertThrow(
          ...
          "Geometry type '"
              << grid_type
              << "' is not a valid geometry type. Valid geometry types are: "
              << "'cartesian', 'annulus', 'chunk' and 'sphere'. "
              << "Please note that the annulus can only be used in 2d and the "
                 "sphere can only be used in 3d.");
    }

    // create paraview file.
    std::cout << "[5/6] Preparing to write the paraview file...                "
                 "                                   \r";
    std::cout.flush();
    ...
    std::cout << "[5/6] Preparing to write the paraview file: stage 1 of 6, "
                 "converting the points                              \r";
    std::cout.flush();
    ...
    std::cout << "[5/6] Preparing to write the paraview file: stage 2 of 6, "
                 "converting the connectivity                              \r";
    std::cout.flush();
    const size_t pow_2_dim = dim == 2 ? 9 : 12;
    ...
    if (dim == 2)
      for (size_t i = 0; i < n_cell; ++i) {
        ...
        connectivity[i * pow_2_dim + 1] =
            static_cast<int>(grid_connectivity[i][1]);
        connectivity[i * pow_2_dim + 2] =
            static_cast<int>(grid_connectivity[i][2]);
        connectivity[i * pow_2_dim + 3] =
            static_cast<int>(grid_connectivity[i][3]);
      }
    else
      for (size_t i = 0; i < n_cell; ++i) {
        ...
        connectivity[i * pow_2_dim + 3] =
            static_cast<int>(grid_connectivity[i][3]);
        connectivity[i * pow_2_dim + 4] =
            static_cast<int>(grid_connectivity[i][4]);
        connectivity[i * pow_2_dim + 5] =
            static_cast<int>(grid_connectivity[i][5]);
        connectivity[i * pow_2_dim + 6] =
            static_cast<int>(grid_connectivity[i][6]);
        connectivity[i * pow_2_dim + 7] =
            static_cast<int>(grid_connectivity[i][7]);
      }
    std::cout << "[5/6] Preparing to write the paraview file: stage 3 of 6, "
                 "creating the offsets                              \r";
    std::cout.flush();
    ...
    std::cout << "[5/6] Preparing to write the paraview file: stage 4 of 6, "
                 "creating the Data set info                              \r";
    std::cout.flush();
    std::vector<vtu11::VtkCellType> types(n_cell, dim == 2 ? 9 : 12);

    // Create tuples with (name, association, number of components) for each
    // data set
    ...
    for (size_t c = 0; c < compositions; ++c) {
      dataSetInfo.emplace_back(
          vtu11::DataSetInfo("Composition " + std::to_string(c),
                             vtu11::DataSetType::PointData, 1));
    }

    std::cout << "[5/6] Preparing to write the paraview file: stage 5 of 6, "
                 "computing the temperatures                              \r";
    std::cout.flush();
    // compute temperature
    ...
    if (dim == 2) {
      pool.parallel_for(0, n_p, [&](size_t i) {
        ...
        temperature_vector[i] =
            world->temperature(coords, grid_depth[i], gravity);
      });
    } else {
      pool.parallel_for(0, n_p, [&](size_t i) {
    ...
    std::cout << "[5/6] Preparing to write the paraview file: stage 6 of 6, "
                 "computing the compositions                              \r";
    std::cout.flush();
    ...
    for (size_t c = 0; c < compositions; ++c) {
      ...
      if (dim == 2) {
        pool.parallel_for(0, n_p, [&](size_t i) {
          std::array<double, 2> coords = {{grid_x[i], grid_z[i]}};
          temp_vector[i] = world->composition(coords, grid_depth[i],
    ...
    std::cout << "[6/6] Writing the paraview file                              "
                 "                                                  \r";
    std::cout.flush();

    vtu11::Vtu11UnstructuredMesh mesh{points, connectivity, offsets, types};
    vtu11::writeVtu(file_without_extension + ".vtu", mesh, dataSetInfo,
                    data_set, vtu_output_format);
